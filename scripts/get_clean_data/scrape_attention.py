...
def get_reddit_data(kw, start_date, end_date):
    ...
    try:
        ...
        for date in date_range:
            ...
            time.sleep(0.05)
            url = f'https://api.pushshift.io/reddit/submission/search?title={kw}&since={current_start}&until={current_end}&limit=0&track_total_hits=true'
    ...
    try:
        ...
        try:
            if result.returncode == 0:
                ...
                return pd.DataFrame(
                    {'kw': [str(kw)], 'date': [start_date], 'value': [-1], 'search_type': [search_type]}, index=[0])
        except Exception as e:
